...
- **Publicação da outbox (`cmd/outbox-publisher`) e consumidor SQS + inbox (`cmd/wager-consumer`) estão implementados**, com testes de integração escritos (`wagerflow_integration_test.go` para o publisher; `wager_consumer_integration_test.go` para o consumidor: mensagem nova, reentrega da mesma mensagem, mesma operação por mensagens diferentes, mensagens concorrentes). **Nenhum dos dois foi executado ainda contra Postgres/SQS reais** — o ambiente onde foram escritos não tem Docker disponível (só `go build`/`go vet`/`go test` sem a tag `integration` foram confirmados; o arquivo de contexto da sessão traz os comandos exatos a rodar antes de considerar este bloco fechado).
